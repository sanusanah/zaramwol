...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('✅ zaramwol working!')
        );
        ...
        if (config.WORKTYPE == 'public') {
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                if (conn.user.jid === '@s.whatsapp.net') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*zarabot Started*', MessageType.text);
                }
            }
            else {
...
*/
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ *zaramwol ERROR FOUND!* ~______~*' +
                                    '\n\n*🧞‍♂️ ' + error + '*\n\n```*Subscribe channel to get updates and to remove errors:* https://youtube.com/channel/UCmg1OY95ymhaOcddKVAsfxg \n Report errors\njoin with +91 \n https://chat.whatsapp.com/H1X6sQsRtCe9W10HkZblle``` ' 
                                    , MessageType.text);
                            }
                        }
                    }
